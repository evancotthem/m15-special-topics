...
CountryGraphs <- function(selectedCountry) {
  country <- filter(gapminder, country == selectedCountry)
  country %>% 
    ggplot(aes(year, lifeExp)) + 
    geom_line() + 
    ggtitle("Full data = ")
  
  country_mod <- lm(lifeExp ~ year, data = country)
  country %>% 
    add_predictions(country_mod) %>%
    ggplot(aes(year, pred)) + 
    geom_line() + 
    ggtitle("Linear trend + ")
  
  country %>% 
    add_residuals(country_mod) %>% 
    ggplot(aes(year, resid)) + 
    geom_hline(yintercept = 0, colour = "white", size = 3) + 
    geom_line() + 
    ggtitle("Remaining pattern")
